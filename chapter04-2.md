...
## 구현
- 1) 포함 객체에 대한 명확한 참조자
  - 자식 구성요소에서 부모를 가리키는 참조자를 관리하면 복합체 구조의 관리를 단순화할 수 있음
  - 부모 객체에 대한 참조자는 주로 Component 클래스에 두는데, Leaf 클래스와 Composite 클래스는 이 Component를 상속받고 있으므로, 두 클래스 모두 참조자를 관리하는 셈
  - 부모의 참조자가 있으면 이런 복합 구조에서는 반복적으로 자신들의 부모에 대한 참조가 가능
  - Composite 클래스의 Add() / Remove() 연산에서만 부모에 대한 참조자를 관리하면, 이를 상속하는 서브클래스에서는 이 코드를 상속 받아서 자동으로 부모에 대한 참조자를 관리할 수 있게 됨
- 2) 구성요소 공유
  - 구성요소를 공유하면 메모리 저장 공간의 필요량을 줄일 수 있음
    - 그러나 하나 이상의 부모를 갖는다면 구성요소를 공유하기 어려움
    - 해결책 중 하나는 자식이 여러 부모를 갖게 하는 것이고 부모들을 다 저장하지 않고도 어떻게 설계를 개선하는지에 대해 플라이급 패턴에서 설명
- 3) Component 인터페이스를 최대화
  - 사용자가 어떤 Leaf, Composite 클래스의 존재를 모르게 하려면 Component 클래스에 Leaf, Composite 에서 정의된 모든 공통의 연산을 다 정의하고 있어야됨
  - 이런 목표가 종종 상속 구조의 중요한 원칙과 충돌할 떄도 있음
    - Component는 서브클래스 Leaf에서 정의하지 않는 연산도 정의
    - Component 클래스의 자식을 처리하는 연산의 기본 구현 사항으로 아무것도 반환하지 않도록 기본 구현을 만들면됨
    - 그리고 Leaf 클래스는 이 구현을 그대로 사용하고, Composite 클래스는 자식을 반환하도록 재정의하면 됨
- 4) 자식을 관리하는 연산 선언
  - 복합체 패턴에서 매우 중요한 관심사는 Composite 클래스 계통 내의 어느 클래스에 이 연산을 (Add(), Remove()) 선언할 것인가 결정하는 것
  - 이 결정은 안정성과 투명성 사이에 양자택일 해야됨
    - 자식을 관리하는 인터페이스를 클래스 계통의 최상위 클래스에 정의하는 경우
      - 서브클래스 모두에게 동일한 인터페이스가 유지되어 사용자에게 인터페이스 투명성을 부여할 수 있음 
      - 반대로 사용자가 Leaf 클래스의 인스턴스에게 Add(), Remove() 연산을 호출하는 의미 없는 해동을 하지 않도록 안정성 유지 비용을 지불해야됨
    - Composite 클래스에만 자식을 관리하는 연산을 정의한다면
      - 사용자는 Leaf 클래스의 인스턴스에 Add(), Remove() 연산을 요청하지 않을 것이므로 안정성을 보장 받음
      - 그러나 Leaf 클래스와 Composite 클래스가 서로 다른 인터페이스를 갖게 되므로 사용자는 동일한 대상으로 간주할 수 없게 됨 (투명성 위반)
    - 안정성 보다 투명성을 강조 하고 싶을때?
      - P. 233 방식으로 처리 (실제 Composite 인 경우에만 Add(), Remove() 사용하도록)
      - 확장성 측면에서 매우 바람직하지 못함
        - 어떤 연산을 호출하기 전에 이 연산을 처리할 수 있는 대상인지(Composite) 다 검사해야됨

- 5) Component가 Component의 리스트를 구현할 수 있을까?
  - Composite에 정의하지 않고 Componenet에서 집합을 관리하는 것으로 이해함
  - 최상위 클래스에 자식 포인터를 정의하는 것은 모든 Leaf 클래스의 인스턴스들도 이 집합을 관리하기 위한 메모리를 정의해야 한다는 것이라 바람직하지 못함
    - Leaf 들은 이 정보를 알 필요 없기 때문 

- 6) 자식 사이의 순서 정하기
  - 자식 간의 순서가 의미 있고 문제가 될 때는 자식에게 접근, 관리하는 인터페이스를 설계 시 자식들의 순서를 관리할 수 있도록 주의를 기울여야 됨
    - 반복자 패턴(Iterator)이 이 부분에서 도움을 줄 수 있음 

- 7) 성능 개선을 위한 캐싱
  - Composite 클래스가 탐색이나 최단 경로 순회의 실제 결과를 임시로 저장
  - 구성요소가 변경되면 부모가 캐싱하는 정보는 의미 없어짐
  - 캐싱을 이용하려면, 현재 저장된 캐시의 내용이 유효한지 아닌지를 확인하는 연산을 정의해야됨 

- 8) 누가 구성요소를 삭제하는 책임을 질까?
  - GC를 제공하지 않는 언어에서는 자식 삭제 시, Composite 클래스가 보통 그 삭제의 책임을 짐
  - 그러나 Leaf 객체가 변경될 수 없는 객체거나 공유될 수 있는 객체라면 예외적으로 삭제할 수 없음

- 9) 구성요소를 저장하기 위해 가장 적당한 데이터 구조는?
  - 연결 리스트, 배열, 트리, 해시 테이블 모두 구현 대상
    - 꼭 일반적인 데이터 구조를 써야 하는 것도 아님
