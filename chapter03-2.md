...
# 생성 패턴에 대한 논의
- 시스템이 생성하는 객체의 클래스로 시스템을 매개변수화 하는 두 가지 방법
  - 1) 객체를 생성하는 클래스를 상속해서 서브클래스로 생성
    - 팩토리 메서드가 이에 속함 
    - 단점은 제품 클래스가 바뀔 때마다 서브클래스 생성해야 함
  - 2) 객체 합성
    - 제품 객체의 클래스를 파악할 책임이 있는 객체를 정의하고 이를 시스템의 매개변수로 만듬
    - 추상 팩토리, 빌더, 원형 패턴의 주요 특징
      - 어쨌든 새로운 '팩토리 객체'를 생성. 이 팩토리 객체는 제품 객체를 생성하는 책임을 갖고 있음
        - 추상 팩토리 패턴 : 여러 클래스의 객체를 생성하는 팩토리
        - 빌더 패턴 : 복잡한 프로토콜을 이용하여 제품을 구축하는 팩토리
        - 원형 패턴 : 원형 객체를 복사하여 제품을 구축하는 팩토리 (팩토리와 원형은 동일한 객체)

- GraphicTool 제품 매개변수화 되는 다양한 방법
  - 팩토리 메서드 패턴
    - Graphic의 서브클래스(product) 각각에 대해 GraphicTool의 서브 클래스(factory)를 생성
    - GraphicTool은 newGraphic() 연산을 갖게됨
  - 추상 팩토리 패턴
    - 여러 GraphicFactory의 클래스 계통이 만들어짐 (AbstractFactory)
    - 클래스 계통은 Graphic(AbstractProduct) 서브클래스별로 한 개씩
    - 각 팩토리는 단지 하나의 제품만 생성
      - CircleFactory는 Circle, LineFactory는 Line 
    - GraphicTool은 적절한 종류의 Graphic을 만들기 위한 팩토리로 매개변수화
  - 원형 패턴 
    - Graphic(Prototype)의 서브클래스가 Clone() 연산을 구현하며, GraphicTool(Client)은 그것이 생성하는 Graphic의 원형으로 매개변수화

- 팩토리 메서드
  - 설계를 사용자가 입맛에 맞게 고칠 수 있도록 해 주면서 그 설계가 복잡해 지지 않게 함
  - 팩토리 메서드 패턴은 새로운 연산만 정의하면 됨
- 추상 팩토리, 원형, 빌더 패턴
  - 팩토리 메서드 설계보다 더 유연할 떄가 많음
  - 팩토리 메서드를 사용해서 시작한 설계에 좀더 유연성이 필요 시, 다른 생성 패턴을 사용하는 설계로 진화

- 다양한 패턴을 알고 있으면, 여러 설계 기준을 놓고 양자택일을 해야 하는 시점에 적절하게 최선의 것을 선택할 수 있음

## 궁금한 부분
- 패턴 구성의 클래스와 객체가 아직 모호한듯
  - 클래스 주로 상속 (정적)
  - 객체 (동적)
